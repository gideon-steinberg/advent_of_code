...
var input2 = @"";
	...
	var NUMBEROFSTEPSALLOWED = 10;
	var MINIMUMNUMBEROFSTEPS = 4;
	...
	while (pointsToProcess.Any())
	{
		...
		if (smallestPoint.i + 1 < height && smallestPoint.direction != Direction.Up)
		{
			...
			var isValid = true;
			while (p1.steps < MINIMUMNUMBEROFSTEPS)
			{
				if (p1.i + 1 < height)
				{
					p1.steps++;
					p1.cost =  p1.cost + map[p1.i + 1, p1.j];
					p1.i = p1.i + 1;
				}
				else
				{
		...
		if (smallestPoint.j + 1 < width && smallestPoint.direction != Direction.Left)
		{
			...
			while (p1.steps < MINIMUMNUMBEROFSTEPS)
			{
				...
				{
					p1.steps++;
					p1.cost =  p1.cost + map[p1.i, p1.j + 1];
					p1.j = p1.j + 1;
				}
				else
				{
