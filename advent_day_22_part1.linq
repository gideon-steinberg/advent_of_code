...
var input2 = @"";
	...
	var supportedBricksBelow = new Dictionary<int, List<int>>();
	var supportedBricksAbove = new Dictionary<int, List<int>>();
	
	foreach (var point in points.Keys)
	{
		supportedBricksBelow.Add(point.id, new List<int>());
		supportedBricksAbove.Add(point.id, new List<int>());
	}
	...
	points.Keys.Where(p => p.canBeRemoved).Count().Dump();
